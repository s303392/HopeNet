...
def main():
    ...
    if args.test:
        ...
        # Verifica il numero di elementi e batch
        ...
        for i, ts_data in enumerate(testloader):
            ...
            predictions3d.append(outputs3d.cpu().data.numpy())

        if i > 0:  # Controlla che i sia stato definito
            print('test error: %.5f' % (running_loss / (i+1)))

            if args.output_predictions:
                predictions3d = np.concatenate(predictions3d, axis=0)
                np.save(args.output_predictions, predictions3d)
                print(f'3D predictions saved to {args.output_predictions}')
        else:
